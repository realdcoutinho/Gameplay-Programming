...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		BehaviorTree* pBehaviourTree = new BehaviorTree(pBlackBoard, new BehaviorAction(BT_Actions::ChangeToWander));
	...
	BehaviorTree* pBehaviorTree = new BehaviorTree(pBlackBoard, new BehaviorSelector(
		{
			new BehaviorSelector({
				...
				{
					new BehaviorConditional(BT_Conditions::IsEnemyNearByLarger),
					new BehaviorAction(BT_Actions::ChangeToFlee)
				}),

				new BehaviorSequence(
				{
					new BehaviorConditional(BT_Conditions::IsAgentNearBorder),
					new BehaviorAction(BT_Actions::ChangeToEvadeBorder)
				}),
				...
				new BehaviorSequence(
				{
					...
					new BehaviorAction(BT_Actions::ChangeToPursuit)
				}),
