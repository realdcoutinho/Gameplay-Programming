...
void App_AgarioGame::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		m_pBlackBoard = CreateBlackboard(newAgent);


		FiniteStateMachine* pStateMachine = new FiniteStateMachine(pWanderState, m_pBlackBoard);
	...
	PursuitSmallAgent* pPursuitSmallAgent = new PursuitSmallAgent();
	m_pStates.push_back(pPursuitSmallAgent);
	...
	IsFoodGone* pIsFoodGone = new IsFoodGone();
	m_pConditions.push_back(pIsFoodGone);

	AgentNearBorder* pAgentNearBorder = new AgentNearBorder();
	m_pConditions.push_back(pAgentNearBorder);

	IsAwayFromBorder* pIsAwayFromBorder = new IsAwayFromBorder();
	m_pConditions.push_back(pIsAwayFromBorder);

	IsAgentWondering* pIsAgentWondering = new IsAgentWondering();
	m_pConditions.push_back(pIsAgentWondering);

	EnemyNearBySmaller* pIsNearbySmaller = new EnemyNearBySmaller();
	m_pConditions.push_back(pIsNearbySmaller);

	EnemyNotCaught* pIsEnemyNotCaught = new EnemyNotCaught();
	m_pConditions.push_back(pIsEnemyNotCaught);

	EnemyCaught* pIsEnemyCaught = new EnemyCaught();
	m_pConditions.push_back(pIsEnemyCaught);
	...
	//pStateMachine->AddTransition(pWanderState, pPursuitSmallAgent, pIsNearbySmaller); //from wander to pursuit


	//pStateMachine->AddTransition(pPursuitSmallAgent, pPursuitSmallAgent, pIsEnemyNotCaught);
	//pStateMachine->AddTransition(pPursuitSmallAgent, pWanderState, pIsEnemyCaught);


	pStateMachine->AddTransition(pWanderState, pSeekFoodState, pFoodNearByCondition); // from wonder to seek food
	//pStateMachine->AddTransition(pSeekFoodState, pWanderState, pIsFoodGone); // from seek to wonder

	//pStateMachine->AddTransition(pWanderState, pEvadeBorderState, pAgentNearBorder); // from wander to evade border
	//pStateMachine->AddTransition(pEvadeBorderState, pWanderState, pIsAwayFromBorder); // from evade border back to wander

	//pStateMachine->AddTransition(pWanderState, pWanderState, pIsAgentWondering); // from wander to wander



	//

	//pStateMachine->AddTransition(pEvadeBorderState, pSeekFoodState, pFoodNearByCondition); //from evade border to seek food

	//pStateMachine->AddTransition(pSeekFoodState, pEvadeBorderState, pAgentNearBorder); // from seek food to evade border
...
{
	//m_pAgentVec;
	m_pBlackBoard->ChangeData("EnemiesVecPtr", &m_pAgentVec);;
...
Blackboard* App_AgarioGame::CreateBlackboard(AgarioAgent* a)
{
	...
	pBlackboard->AddData("FoodTarget", static_cast<AgarioFood*>(nullptr));
	pBlackboard->AddData("BorderSize", &m_TrimWorldSize);

	pBlackboard->AddData("EnemiesVecPtr", &m_pAgentVec);
	pBlackboard->AddData("Enemy", static_cast<AgarioAgent*>(nullptr));
	
	pBlackboard->AddData("WanderBehaviour", &m_pSmartAgent->m_pWander);
