...
{
	//TODO
	bool isValid = pBlackBoard->GetData("Agent", m_pAgent);
	if (isValid == false || m_pAgent == nullptr)
	{
		return;
	}

	isValid = pBlackBoard->GetData("FoodTarget", m_pFood);
	if (isValid == false || m_pFood == nullptr)
	{
		return;
	}
	...
	Vector2 foodPosition{ m_pFood->GetPosition() };

	m_pAgent->SetToSeek(foodPosition);
...
void PursuitSmallAgent::Update(Elite::Blackboard* pBlackBoard, float deltaTime) 
{
	AgarioAgent* pAgent;
	bool isValid = pBlackBoard->GetData("Agent", pAgent);
	if (isValid == false || pAgent == nullptr)
...
bool EnemyNearBySmaller::Evaluate(Blackboard* pBlackBoard) const
{
	...
	isValid = pBlackBoard->GetData("EnemiesVecPtr", pEnemiesVec);
	if (isValid == false || pEnemiesVec == nullptr)
	{
		return false;
	}
