...
namespace Elite 
{
	...
	inline bool IsPointInCircle(const Vector2 point,const Vector2 center, const float radius)
	{
		float squaredDist{ (point.x - center.x) * (point.x - center.x) + (point.y - center.y) * (point.y - center.y) };
		float squaredRadius{ radius * radius };
		return (squaredRadius >= squaredDist);
	}

	inline bool IsPointInRect(const Vector2 point, const std::vector<Elite::Vector2> rect)
	{
		return (point.x >= rect[0].x &&
			point.x <= rect[3].x &&
			point.y >= rect[0].y &&
			point.y <= rect[1].y);
	}
