	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				Vector2 updatedRight{ portal.Line.p1 - apexPos };

				float inwardsRight{ rightLeg.Cross(updatedRight) };

				if (inwardsRight >= 0.f)
				{
					float inwardsLeft{ updatedRight.Cross(leftLeg) };
					if (inwardsLeft < 0.f)
					{
				...
				Vector2 updatedLeft{ portal.Line.p2 - apexPos };

				float inwardsLeft{ leftLeg.Cross(updatedLeft) };
				if (inwardsLeft <= 0.f)
				{
					float inwardRight{ updatedLeft.Cross(rightLeg) };
					if (inwardRight > 0.f)
					{
